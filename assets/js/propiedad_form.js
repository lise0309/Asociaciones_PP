/**
 * propiedad_form.js
 * PP Bienes Raíces — assets/js/propiedad_form.js
 * Mapa: Leaflet (igual que detalle.php)
 */
...
function initMapa() {
  ...
  const icono = L.divIcon({
    html: `
      <div style="
        ...
        background:#1A1953;border:3px solid #FFD45A;
        transform:rotate(-45deg);
        box-shadow:0 4px 14px rgba(26,25,83,.4);
  ...
  marcador = L.marker([lat, lng], { icon: icono, draggable: true })
    .addTo(mapaLeaflet)
    .bindPopup('<strong>Arrastra para ajustar</strong><br>o haz clic en el mapa');
...
function actualizarCoordsDisplay(lat, lng) {
  ...
  if (el) {
    el.textContent = `${parseFloat(lat).toFixed(5)}° N, ${Math.abs(parseFloat(lng)).toFixed(5)}° O`;
  }
}
...
function confirmarUbicacion() {
  if (tempLat !== null && tempLng !== null) {
    ...
    if (texto) {
      texto.textContent = `${currentLat.toFixed(5)}° N, ${Math.abs(currentLng).toFixed(5)}° O`;
    }

    // Marcar preview como "tiene coords"
  ...
  if (!mapaLeaflet) {
    ...
    setTimeout(() => {
      ...
      if (currentLat && currentLng) {
        mapaLeaflet.setView([currentLat, currentLng], 13);
      }
    }, 150);
  }
}
...
// Si ya hay coordenadas al cargar, marcar el preview
(function () {
...
function handleFiles(files) {
  ...
  const permitidas = max - actuales;

  if (files.length > permitidas) {
    mostrarToast(`Máximo ${max} fotos. Solo se agregarán ${permitidas}.`, 'error');
  ...
  files.forEach(file => {
    ...
    reader.onload = e => agregarFotoPreview(e.target.result, file.name);
...
function onDrop(e) {
  ...
  if (dragSrcEl && dragSrcEl !== this) {
    ...
    if (srcIdx < dstIdx) fotosGrid.insertBefore(dragSrcEl, this.nextSibling);
...
// Aplicar drag a fotos existentes
...
form?.addEventListener('submit', function (e) {
  ...
  document.getElementById('btnGuardar')?.classList.add('loading');
});
